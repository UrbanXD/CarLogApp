...
export class SupabaseStorageAdapter implements StorageAdapter {
    ...
    async uploadFile(
        ...
        const res = await this.options.client.storage
            .from(BaseConfig.SUPABASE_CAR_IMAGES_BUCKET)
    ...
    async downloadFile(filePath: string) {
        ...
        const { data, error } = await this.options.client.storage.from(BaseConfig.SUPABASE_CAR_IMAGES_BUCKET).download(filePath);
        if (error) {
            throw error;
        }
    ...
    async deleteFile(uri: string, options?: { filename?: string }): Promise<void> {
        ...
        const { data, error } = await this.options.client.storage.from(BaseConfig.SUPABASE_CAR_IMAGES_BUCKET).remove([filename]);
        if (error) {
